defmodule ExcyteWeb.Agent.SendToClient do
  ...
  def update(assigns, socket) do
    ...
    activities =
      Activities.fetch_insight_activities(%{
    ...
    last_sent =
      if length(activities) > 0,
        do: hd(Enum.filter(activities, fn a -> a.name === "sent" end)),
        else: nil

    subject_type_text =
      case assigns.insight.type do
        "cma" -> "has sent you a Comparative Market Analysis for your home"
        "cma_auto" -> "has sent you a Comparative Market Analysis for your home"
    ...
    message_type_text =
      case assigns.insight.type do
        ...
        "cma_auto" -> "Comparative Market Analysis for you to browse at your convenience"
  ...
  def handle_event("validate-email-form", %{"send_to" => f}, %{assigns: a} = socket) do
    valid =
      if String.match?(a.email, ~r/^[^\s]+@[^\s]+$/) && String.length(a.subject) > 0 &&
           String.length(a.client_name) > 0 do
        true
      else
        false
      end
  ...
  def handle_event("submit-email-form", %{"send_to" => f}, %{assigns: a} = socket) do
    if a.valid do
      ...
      {:noreply,
       assign(socket,
         errors: [%{message: "Please make sure you have a name, valid email, and subject."}]
       )}
    end
  end
end
