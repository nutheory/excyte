defmodule ExcyteWeb.Agent.Dashboard do
  ...
  use ViewportHelpers
  alias Excyte.{Contacts, Insights}
  ...
  def handle_info({:save_client, attrs}, %{assigns: a} = socket) do
    case Contacts.create_client(attrs) do
      {:ok, %{new_client: client}} ->
        {:noreply,
         put_flash(
           socket,
           :info,
           "#{Utilities.insight_type_to_name(a.insight_to_send.type)} sent to #{client.email}"
         )
