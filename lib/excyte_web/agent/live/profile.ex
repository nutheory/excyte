defmodule ExcyteWeb.Agent.Profile do
  ...
  def handle_event("save", %{"profile" => profile_params}, %{assigns: a} = socket) do
    ...
    with {:ok, _} <-
           Agents.update_profile(a.profile, Map.merge(params, %{"updated_by_user" => true})),
         {:ok, _} <-
           Accounts.update_user(a.current_user.id, %{current_avatar: a.profile.photo_url}) do
      {:noreply,
       put_flash(socket, :info, "Profile updated successfully") |> push_redirect(to: a.return_to)}
    else
      {:error, %Ecto.Changeset{} = changeset} ->
        {:noreply, assign(socket, to_form(changeset))}
    end
  ...
  defp maybe_attempt_prefill?(profile, mls) do
    if profile.updated_by_user === false && Map.has_key?(mls, :member_key) && mls.member_key do
      ...
      mls_contacts =
        Enum.map(mls_details.contacts, fn cnt ->
          %ContactItem{
      ...
      contacts =
        if length(profile.contact_items) > 0,
          do: profile.contact_items,
          else: [%ContactItem{temp_id: Utilities.get_temp_id()}]
