defmodule ExcyteWeb.Brokerage.ManageTeam do
  ...
  def mount(_params, %{"user_token" => token}, socket) do
    ...
    acc =
      Accounts.get_account_with_brokerage_agents(%{
        brokerage: cu.brokerage_id,
        account: cu.account_id
      })
  ...
  def handle_event("delete_user", %{"user-id" => uid}, %{assigns: a} = socket) do
    ...
    if Utilities.authorized?(a.current_user.brokerage_role) do
      case Accounts.delete_user(%{brokerage_id: a.current_user.brokerage_id, id: id}) do
        ...
        _ ->
          {:noreply,
           put_flash(socket, :error, "Could not delete selected user. We'll look into why.")}
      end
    else
  ...
  def handle_event("validate", %{"user" => ivt} = f, %{assigns: a} = socket) do
    ue =
      if Enum.member?(f["_target"], "invite_message") && ivt["invite_message"] !== "",
        do: true,
        else: false
  ...
  def handle_event("reset-message", _, %{assigns: a} = socket) do
    cs =
      Accounts.change_invitation(
        %User{},
        Map.merge(a.changeset.changes, %{
          "invite_message" =>
            default_message(%{
  ...
  def handle_event("save", %{"user" => ivt}, %{assigns: a} = socket) do
    if Utilities.authorized?(a.current_user.brokerage_role) do
      if length(a.members) <= a.account.agent_limit do
        ...
        with {:ok, invite} <-
               Accounts.create_brokerage_invitation(
                 Map.merge(ivt, %{
          ...
          {:noreply,
           put_flash(socket, :info, "Invite sent to #{ivt["email"]}.")
           |> assign(members: [invite | a.members])}
        else
          {:error, %Ecto.Changeset{} = err_cs} ->
            {:noreply, assign(socket, form: to_form(err_cs))}
        end
      else
        {:noreply,
         assign(socket, errors: [%{message: "You have maxed out your agent sign ups."}])}
      end
    else
      {:noreply,
       assign(socket,
         errors: [%{message: "You are not authorized to invite agents to this account."}]
       )}
    end
  end
