defmodule ExcyteWeb.Insight.Customize do
  ...
  def mount(%{"insight_id" => id}, _sesh, %{assigns: %{current_user: cu}} = socket) do
    ...
    case Insights.get_minimal_insight(id, cu.id) do
      ...
      {:error, err} ->
        Activities.handle_errors(err, "ExcyteWeb.Insight.CustomizeCma")
  ...
  def handle_event("publish", %{"name" => name}, %{assigns: a} = socket) do
    ...
    {:noreply,
     ...
     |> put_flash(
       :info,
       "#{Utilities.insight_type_to_name(published.insight.type)} was created and published successfully."
     )
  ...
  def handle_event("sort-listings", %{"listings" => [_ | _] = listings}, %{assigns: a} = socket) do
    rearranged =
      Enum.map(listings, fn %{"id" => id, "position" => pos} ->
        Enum.find(a.listings, fn %{"excyte_data" => %{"temp_id" => tid}} ->
          String.to_integer(id) === tid
        end)
  ...
  def handle_event("toggle-enabled", %{"id" => id}, %{assigns: a} = socket) do
    sections =
      Enum.map(a.sections, fn st ->
        if st.temp_id === String.to_integer(id),
          do: Map.merge(st, %{enabled: !st.enabled}),
          else: st
      end)
      ...
      Enum.map(a.listings, fn st ->
        if st["excyte_data"]["temp_id"] === String.to_integer(id) do
          Map.update!(
            st,
            "excyte_data",
            &Map.merge(&1, %{"enabled" => !st["excyte_data"]["enabled"]})
          )
        else
          st
        end
  ...
  defp publish_sections(assigns) do
    Enum.map(assigns.sections, fn s ->
      %{
        ...
        name: s.name,
        content:
          if(Map.has_key?(s, :content),
            do: Map.drop(s.content, [:__meta__, :__struct__, :brokerage, :uploaded_by]),
            else: nil
          ),
  ...
  defp set_default_name(ins) do
    ...
    case ins.type do
      "cma_auto" ->
        "#{hd(ins.content.listings)["street_number"]} #{hd(ins.content.listings)["street_name"]}"

      "cma" ->
        "#{ins.property.street_number} #{ins.property.street_name}"
      ...
      url =
        case type do
          "buyer_tour" ->
            ...
            q =
              Enum.reduce(listings, init, fn lst, acc ->
            ...
            with {:ok, %{body: image}} <-
                 ...
                 _ <-
                   ExAws.request!(
                     ExAws.S3.put_object(
                       "excyte",
                       "insights/buyer_tour_covers/#{insight.id}/cover.png",
                       image,
                       acl: :public_read,
