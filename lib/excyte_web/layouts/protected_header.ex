      ...
      <div class="bg-gray-100 backdrop-blur-sm max-w-screen-xl mx-auto h-12 lg:h-18 border-b border-wgray-300 bg-opacity-95 flex">
        ...
        <div class="lg:flex-auto h-12 lg:h-18 flex items-center justify-between px-4 lg:px-6 lg:mx-6">
          ...
          <div class="relative" x-data="{ open: false }" @keydown.escape="open = false" @click.outside="open = false">
            <div class="relative text-lg rounded-full flex items-center justify-center text-bgray-100 font-bold border-teal-500 bg-bgray-800 border-2 lg:border-4 uppercase h-9 w-9 lg:w-14 lg:h-14 z-30 cursor-pointer avatar" @click="open = !open">
              <%= if @current_user.current_avatar do %>
                <div class="rounded-full w-full h-full bg-cover" style={"background-image: url(#{@current_user.current_avatar});"}></div>
              <% else %>
            ...
            >
              ...
              <%= for mls <- @mls_options do %>
                ...
                <% end %>
              <% end %>
              <li class="pt-1 border-t border-bgray-300"><.link navigate={~p"/settings"} class="block py-2 px-4 hover:bg-blue-100">Settings</.link></li>
              <li class=""><.link href={~p"/log_out"} method="delete", class="block py-2 px-4 hover:bg-blue-100" >Log out</.link></li>
            </ul>
          </div>
        </div>
      </div>
