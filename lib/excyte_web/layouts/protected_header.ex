defmodule ExcyteWeb.ProtectedHeader do
  ...
  def handle_event("switch-mls", %{"mls-id" => mid}, %{assigns: assigns} = socket) do
    ...
    {:ok, user} =
      Accounts.update_user(assigns.current_user.id, %{
        current_mls: %{
          id: mls.id,
  ...
  def render(assigns) do
    ~H"""
    <nav class="protected-header fixed top-0 z-30 lg:z-40 w-full h-12 lg:h-18">
      <div class="bg-gray-100 backdrop-blur-sm max-w-screen-xl mx-auto h-12 lg:h-18 border-b border-wgray-300 bg-opacity-95 flex">
        <div
          x-cloak
          class="block lg:hidden w-12 mr-4 pl-4 relative"
          x-data="{ navMenuOpen: false }"
          @keydown.escape="navMenuOpen = false"
          @click.outside="navMenuOpen = false"
        >
          <div @click="navMenuOpen = !navMenuOpen" class="h-12 w-8 pt-2 cursor-pointer">
            <svg
          ...
          >
            <div class="w-full bg-gray-800 text-bgray-100 rounded">
              <div class="flex flex-col pb-2">
                <.link navigate={~p"/auth/dash"} class="block">
                  <div class="p-4 hover:bg-bgray-700">
                    <h3 class="text-lg font-bold leading-5 text-blue-600 mb-0">Dash</h3>
        ...
        <div class="flex-1 lg:flex-none flex justify-center items-center lg:w-44 xl:w-48 lg:pl-6 cursor-pointer">
          <.link navigate={~p"/auth/dash"}>
            <svg
              class="w-18 lg:w-36 h-8 lg:h-14 mx-auto"
              viewBox="0 0 353 114"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
            ...
            >
              <defs>
                ...
                <path
                  d="M326.218353,0 L329.380675,3.70372092 C331.525918,14.196289 338.95051,22.7329176 348.716951,26.3393846 L349,26.670578 L320.28257,79.9979044 C316.904559,75.0960701 311.327683,71.862988 304.999774,71.7645869 L326.218353,0 Z"
                  id="path-5"
                >
                </path>
                <filter
        ...
        <div class="lg:flex-auto h-12 lg:h-18 flex items-center justify-between px-4 lg:px-6 lg:mx-6">
          <div class="hidden lg:block w-full">
            <div class="flex">
              ...
              <.link navigate={~p"/auth/properties/overview"} class="block">
                <div class="mt-1 mr-6">
                  <h3 class="text-lg font-bold leading-5 text-cyan-600 mb-0">Pocket Listings</h3>
                  <p class="text-xs text-bgray-400">Your Mini MLS</p>
                </div>
              </.link>
              <.link navigate={~p"/auth/contacts/overview"} class="block">
                <div class="mt-1 mr-6">
                  <h3 class="text-lg font-bold leading-5 text-teal-600 mb-0">Contacts</h3>
                  <p class="text-xs text-bgray-400">Leads, Clients, Etc.</p>
                </div>
              </.link>
            </div>
          </div>
          ...
          >
            <div
              class="relative text-lg rounded-full flex items-center justify-center text-bgray-100 font-bold border-teal-500 bg-bgray-800 border-2 lg:border-4 uppercase h-9 w-9 lg:w-14 lg:h-14 z-30 cursor-pointer avatar"
              @click="open = !open"
            >
              <%= if @current_user.current_avatar do %>
                <div
                  class="rounded-full w-full h-full bg-cover"
                  style={"background-image: url(#{@current_user.current_avatar});"}
                >
                </div>
              <% else %>
            ...
            <ul
              ...
              x-transition:leave-start="transform opacity-100 scale-100"
              x-transition:leave-end="transform opacity-0 scale-95"
              class=" w-60 absolute top-12 right-0 bg-white shadow rounded border border-gray-200 py-1 z-20"
            >
              <li class="py-2 px-4">
                <div class="flex">
                  ...
                  <div class="flex-1 ml-2">
                    ...
                    <p class="text-tiny uppercase leading-none">Current MLS</p>
                  </div>
                </div>
              </li>
              <%= for mls <- @mls_options do %>
                <%= if mls.id !== @current_mls_id do %>
                  <li
                    ...
                    phx-click="switch-mls"
                    phx-value-mls-id={mls.id}
                  >
                    <div class="flex">
                ...
                <% end %>
              <% end %>
              <li class="pt-1 border-t border-bgray-300">
                <.link navigate={~p"/settings"} class="block py-2 px-4 hover:bg-blue-100">
                  Settings
                </.link>
              </li>
              <li class="">
